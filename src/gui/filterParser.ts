...
const getTerms = (query: string, validFilters: Set<string>): Term[] => {
  ...
  for (let i = 0; i < query.length; i++) {
    ...
    if (
      ...
      !inQuote &&
      !inTerm &&
      (validFilters.has(currentTerm.toLowerCase()) ||
        (currentTerm[0] === '-' && validFilters.has(currentTerm.toLowerCase().substr(1, currentTerm.length))))
    ) {
...
const parseQuery = (query: string): Term[] => {
  ...
  // validation
  let incorrect = result.filter((term) => term.name === 'type' || term.name === 'iscompleted').find((x) => x.negated)
  if (incorrect) throw new Error(`${incorrect.name} can't be negated`)

  incorrect = result.filter((term) => term.name === 'type').find((x) => x.value !== 'note' && x.value !== 'todo')
  if (incorrect) throw new Error('The value of filter "type" must be "note" or "todo"')

  incorrect = result.filter((term) => term.name === 'iscompleted').find((x) => x.value !== '1' && x.value !== '0')
