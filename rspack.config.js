...
function validatePackageJson() {
  ...
  if (!content.keywords || content.keywords.indexOf('joplin-plugin') < 0) {
    console.warn(
      chalk.yellow(
        `WARNING: To publish the plugin, the package keywords should include "joplin-plugin" (found "${JSON.stringify(content.keywords)}") in ${packageJsonPath}`,
      ),
    )
  }
...
function validateScreenshots(screenshots) {
  ...
  for (const screenshot of screenshots) {
    ...
    if (fileSize > fileMaxSize)
      throw new Error(`Max screenshot file size is ${fileMaxSize}KB. ${screenshotPath} is ${fileSize}KB`)
  }
}
    ...
    rules: [
      ...
      },
      {
        ...
        options: {
          ...
          env: {
            targets: 'Chrome >= 95', // browser compatibility
          },
        },
      },
      // {
...
function buildExtraScriptConfigs(userConfig) {
  ...
  for (const scriptName of userConfig.extraScripts) {
    ...
    output.push({ ...extraScriptConfig, entry: scriptPaths.entry, output: scriptPaths.output })
  }
